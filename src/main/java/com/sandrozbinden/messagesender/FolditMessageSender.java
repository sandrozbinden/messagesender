    ...
    public void sendMessage(String message, FolditUser user, String sessionID) {
        try {
            ...
            Response response = Jsoup.connect(Foldit.BASE_NEW_MESSAGE_URL + "/" + user.getId()).followRedirects(true)
                    ...
                    .header("Content-Type", "application/x-www-form-urlencoded").header("Accept-Encoding", "gzip,deflate").header("Cache-Control", "max-age=0")
                    .header("Connection", "keep-alive")
                    .userAgent("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.111 Safari/537.36")
                    .cookie(FolditLogin.COOKIE_SESSION_NAME, sessionID).cookie("has_js", "1").timeout(20000).method(Method.GET)
                    .data("recipient", user.getUserName()).data("subject", " Subject").data("body", message).data("op", "Send message")
                    .data("form_build_id", newMessageDocument.select("[name=form_build_id]").val())
                    .data("form_token", newMessageDocument.select("[name=form_token]").val())
