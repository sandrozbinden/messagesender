...
//            	sendMessage(message.getMessage(eternaUser.getUserName()), eternaUser, eternaLogin.getSessionID());
	...
	public void sendMessage(String message, EternaUser user, String sessionID) {
		...
		Response playerResponse = Jsoup.connect(playerURL).cookie(EternaLogin.COOKIE_SESSION_NAME, sessionID)
	            .timeout(20000).execute();
		
		logger.debug(playerResponse.parse().toString());
				
		
		Response response = Jsoup.connect(Eterna.BASE_NEW_MESSAGE_URL + "/")
	            ...
	            .cookie(EternaLogin.COOKIE_SESSION_NAME, sessionID)
	            .timeout(20000)
	            .method(Method.GET)
	            .data("target_uid", user.getId())
	            .data("body", message)
